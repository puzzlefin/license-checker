        ...
        Object.keys(restricted).forEach(function(item) {
            if (toCheckforFailOn.length > 0) {
                if (toCheckforFailOn.indexOf(restricted[item].licenses) > -1) {
                    console.error('Found license defined by the --failOn flag: "' + item + ":" + restricted[item].licenses + '". Exiting.');
                    process.exit(1);
                }
            }
            if (options.failOnCopyleft) {
                if (copyLeftLicenses.includes(restricted[item].licenses)) {
                    console.error('Found license defined by the --failOnCopyleft flag: "' + item + ":" + restricted[item].licenses + '". Exiting.');
                    process.exit(1);
                }
            }
